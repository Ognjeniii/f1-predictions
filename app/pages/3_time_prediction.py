...
with st.form("prediction_form"):
    ...
    with col1:
        season = st.number_input("Season", 2020, 2025, 2024)

    with col2:
        round_number = st.number_input("Round", 1, 30, 1)

    with col3:
        event_name = st.text_input("Event Name", value="Singapore Grand Prix")
    ...
    with col3:
        team = st.text_input("Team", value="Mercedes")
    ...
    with col1:
        pit_in = st.number_input("Pit In Time (sec)", 0.0, value=0.0)

    with col2:
        pit_out = st.number_input("Pit Out Time (sec)", 0.0, value=0.0)
...
if submit:
    try:
        # ✅ Praviš dataframe od user input-a
